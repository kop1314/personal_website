...
void run_server(char *port) {
  ...
  while(!endSession){
    ...
    pthread_t tid;
    pthread_create(&tid, NULL, process_client, (void*)(long)i);
  }
...
void write_to_clients(const char *message, size_t size) {
  ...
  pthread_mutex_lock(&mutex);
  for (int i = 0; i < MAX_CLIENTS; i++) {
    if (clients[i] != -1 && clients[i] == c_fd) {
      ssize_t retval = write_message_size(size, clients[i]);
      if (retval > 0) {

	char* new_mess = strdup(message);
	char* token = strtok(new_mess, "/");
	//printf("token is %s size is %zu\n", token, strlen(token));
	retval = write_all_to_socket(clients[i], token, size);
	free(new_mess);
      }
      if (retval == -1) {
	perror("write(): ");
